...
public class AddPartsAdd extends FrameActivity {
	...
	private String msgStr,pjid,pjmc,pjfy,fbf;
...
//				}
				...
				map.put("dj", "".equals(tv_dj.getText().toString())?"0":tv_dj.getText().toString());
				map.put("sfhs", sfhs);
				list.add(map);
		...
		tv_xl.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				Intent intent1 = new Intent(getApplicationContext(),
						com.single.activity.util.DataFilterActivity.class);
	...
	protected void getWebService(String s) {
		if ("getdj".equals(s)) {
			try {
				String str = fbf+"*"+pjid+"*"+fbf+"*"+pjid+"*"+pjid;
				JSONObject jsonObject = callWebserviceImp.getWebServerInfo(
						"_PAD_HPXX_BJDJ", str, "uf_json_getdata",
						getApplicationContext());
				String flag = jsonObject.getString("flag");
				if (Integer.parseInt(flag) > 0) {
					JSONArray jsonArray = jsonObject.getJSONArray("tableA");
					JSONObject temp = jsonArray.getJSONObject(0);
					pjfy = temp.getString("bjdj");
				} else {
					pjfy = "0";
				}
