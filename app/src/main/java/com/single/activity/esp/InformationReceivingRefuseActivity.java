...
public class InformationReceivingRefuseActivity extends FrameActivity {
	...
	private String flag, jdyy, jjjdrz, khbm, khbmv, zbh;
	...
	protected void initListeners() {
		...
		OnClickListener backonClickListener = new OnClickListener() {
			...
			public void onClick(View v) {
				...
				case R.id.confirm:
					if ("".equals(khbmv)) {
						dialogShowMessage("该区没有组长，请联系服务工程师", null, null);
						return;
					}
					jdyy = "";
					int num = ll_checkboxs.getChildCount();
					for (int i = 0; i < num; i++) {
						LinearLayout ll = (LinearLayout) ll_checkboxs
								.getChildAt(i);
					...
					String ts = "当前已拒单数：" + curr_jdgs
							+ "次，根据平台星级评定标准，拒单扣2分，确认拒绝接单？";
					if (curr_jdgs >= 3) {
						ts = "当前已拒单数：" + curr_jdgs
								+ "次，拒单已累计满3次，平台可能暂停派发工单，确认拒绝接单？";
					}
				...
				case R.id.iv_telphone_kh:
					Call(tv_phone_kh.getText().toString(),zbh);
					break;
				}
	...
	protected void getWebService(String s) {
		...
		if ("query".equals(s)) {
			try {
				...
				JSONObject jsonObject_jdgs = callWebserviceImp
						.getWebServerInfo("_PAD_GDJJCS", DataCache.getinition()
								.getUserId(), "uf_json_getdata", this);
				JSONObject jsonObject_khbm = callWebserviceImp
						.getWebServerInfo("_PAD_JDXY_JJJD_CXRY", khbm,
								"uf_json_getdata", this);

				JSONArray jsonArray = jsonObject_khbm.getJSONArray("tableA");
				flag = jsonObject_khbm.getString("flag");
				if (Integer.parseInt(flag) > 0) {
					JSONObject temp = jsonArray.getJSONObject(0);
					khbmv = temp.getString("rybm");
				} else {
					khbmv = "";
				}

				jsonArray = jsonObject_jdgs.getJSONArray("tableA");
		...
		if (s.equals("submit")) {// 提交
			try {
				...
				String sql = "update shgl_ywgl_fwbgszb set djzt='1.5'"
						+ ",clfs='1',slsj=sysdate,chjdrz=" + czrz3
						+ ",jjgdyy='" + jdyy + "',pgdx = '" + khbmv
		...
		public void handleMessage(Message msg) {
			...
			case 3:
				for (int i = 0; i < jdyy_list.size(); i++) {
					LinearLayout ll_box = (LinearLayout) LayoutInflater.from(
							getApplicationContext()).inflate(
							R.layout.include_checkbox, null);
