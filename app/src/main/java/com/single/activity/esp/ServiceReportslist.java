...
public class ServiceReportslist extends FrameActivity {
	...
	protected void initVariable() {
		...
		from = new String[] { "textView1","网点名称", "zbh", "timemy", "datemy" };
			...
			try {
				...
				if (Integer.parseInt(flag) > 0) {
					for (int i = 0; i < jsonArray.length(); i++) {
						...
						item.put("yysj",temp.getString("yuysj"));
						item.put("bz",
						...
						item.put("khbmmc",
								temp.getString("main_jcsj_dqb_khbmmc"));
						...
						item.put("rzsbmc", temp.getString("rzsbmc"));//认证设备类型
						...
						item.put("smsf", temp.getString("shgl_ywgl_fwbgszb_fwgcs"));// 服务咨询师电话
						item.put("timemy",// 时间
