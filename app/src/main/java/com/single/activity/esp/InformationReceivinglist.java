...
public class InformationReceivinglist extends FrameActivity {
	...
	protected void initVariable() {
		...
		from = new String[] {"textView1", "wdmc", "zbh", "timemy", "datemy" };
			...
			try {//_PAD_SHGL_PGDCZ
				...
				if (Integer.parseInt(flag) > 0) {
					for (int i = 0; i < jsonArray.length(); i++) {
						...
						item.put("wdmc", temp.getString("ccgl_wlsjb_bzr"));//w网点
						...
						item.put("khbmmc", temp.getString("main_jcsj_dqb_khbmmc"));
						item.put("ds", temp.getString("main_jcsj_dqb_ds"));
						item.put("xxdz", temp.getString("shgl_ywgl_fwbgszb_khxxdz"));
