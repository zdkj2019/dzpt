...
import com.single.activity.esp.ChooseActivity;
...
public class WbdbckMxActivity extends FrameActivity {
	...
	private ArrayList<Map<String, String>> data_kfck,data_kfrk, data_cwck,data_cwrk,data_hp; // 所属片区缓存数据
	private String flag,chooseType,currkf,currcw;
	...
	protected void initListeners() {
		...
		confirm.setOnClickListener(new OnClickListener() {
			...
			@Override
			public void onClick(View v) {
				if(isNotNull(edit_ckkfmc)&&isNotNull(edit_ckcwmc)&&isNotNull(edit_rkkfmc)&&
						isNotNull(edit_rkcwmc)&&isNotNull(edit_hpmc)&&isNotNull(edit_sl)){
					
					if(Integer.parseInt(edit_sl.getText().toString())==0){
						toastShowMessage("数量不能为0");
						return;
					}
					if(Integer.parseInt(edit_sl.getText().toString())>Integer.parseInt(tv_dqkc.getText().toString())){
						toastShowMessage("数量不能大于当前库存数量");
						return;
					}
					
					Map<String, String> map = new HashMap<String, String>();
					map.put("ckkfmc_id", "id_"+edit_ckkfmc.getTag().toString());//json 转0开头的字符串会自动去掉0
					map.put("ckkfmc_name", edit_ckkfmc.getText().toString());
					map.put("ckcwmc_id", "id_"+edit_ckcwmc.getTag().toString());
					map.put("ckcwmc_name", edit_ckcwmc.getText().toString());
					map.put("rkkfmc_id", "id_"+edit_rkkfmc.getTag().toString());
					map.put("rkkfmc_name", edit_rkkfmc.getText().toString());
					map.put("rkcwmc_id", "id_"+edit_rkcwmc.getTag().toString());
					...
					map.put("dw", tv_dw.getText()==null?"":tv_dw.getText().toString());
					map.put("sl", edit_sl.getText().toString());
					map.put("dqkc", tv_dqkc.getText()==null?"0":tv_dqkc.getText().toString());
					map.put("bz", edit_bz.getText()==null?"":edit_bz.getText().toString());
		...
		edit_ckcwmc.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				chooseType = "ckcwmc";
				currkf = edit_ckkfmc.getTag().toString();
				if("".equals(currkf)){
		...
		edit_rkcwmc.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				chooseType = "rkcwmc";
				currkf = edit_rkkfmc.getTag().toString();
				if("".equals(currkf)){
					toastShowMessage("请选择入库库房！");
					return;
				}
				ArrayList<Map<String, String>> currdata = new ArrayList<Map<String,String>>();
				for(int i=0;i<data_cwrk.size();i++){
					Map<String, String> temp = data_cwrk.get(i);
					if(currkf.equals(temp.get("sjjg"))){
						currdata.add(temp);
					}
				}
				Intent intent = new Intent(WbdbckMxActivity.this,
						ChooseActivity.class);
				intent.putExtra("data", currdata);
				startActivityForResult(intent, 1);
			}
		});
	...
	protected void getWebService(String s) {
		...
		if ("query".equals(s)) {
			try {
				JSONObject jsonObject_kfrk = callWebserviceImp.getWebServerInfo(
						"_PAD_CFCWCX2", "",
				...
				if (Integer.parseInt(flag) > 0) {
					try {
						...
						data_kfrk = (ArrayList<Map<String, String>>) JSONObjectParser
								.jsonToListByJson(array);
						ArrayList<Map<String, String>> data_kf = new ArrayList<Map<String,String>>();
						ArrayList<Map<String, String>> data_cw = new ArrayList<Map<String,String>>();
						for(int i=0;i<data_kfrk.size();i++){
							Map<String, String> temp = data_kfrk.get(i);
							Map<String, String> newMap = new HashMap<String, String>();
							String bmxz = temp.get("bmxz");
							newMap.put("id", temp.get("jgbm"));
							newMap.put("name", temp.get("jgmc"));
							newMap.put("sjjg", temp.get("sjjg"));
							if("20".equals(bmxz)){
								data_kf.add(newMap);
							}else{
								data_cw.add(newMap);
				...
				JSONObject jsonObject_kfck = callWebserviceImp.getWebServerInfo(
						"_PAD_KFCK_KF", DataCache.getinition().getUserId(),
						"uf_json_getdata", this);
		...
		if ("getcw".equals(s)) {
			try {
				JSONObject jsonObject_cw = callWebserviceImp.getWebServerInfo(
						"_PAD_KFCK_CW", DataCache.getinition().getUserId()+"*"+currkf,
						"uf_json_getdata", this);
		...
		if ("gethp".equals(s)) {
			try {
				JSONObject jsonObject_hp = callWebserviceImp.getWebServerInfo(
						"_PAD_DBCKHPXX", DataCache.getinition().getUserId()+"*"+currcw,
						"uf_json_getdata", this);
